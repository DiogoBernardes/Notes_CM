...
class AddFragment : Fragment() {
    ...
    private fun addNote() {
        ...
        if(noteText.isEmpty()) {
            Toast.makeText(view?.context, R.string.empty_note, Toast.LENGTH_LONG).show()
        }
        else {
            ...
            Toast.makeText(requireContext(), R.string.recorded_successfully, Toast.LENGTH_LONG).show()
            findNavController().navigate(R.id.action_addFragment_to_listFragment)
        }
    }
}